...
public void ReleaseAssertVersionFilesAgree(string releaseVersion, List<string> errors)
{
    ...
    else
    {
        ...
        // AssemblyVersion/AssemblyFileVersion carry the numeric identity only
        // (a SemVer2 label in either is CS7034); AssemblyInformationalVersion
        // carries the full release string, prerelease label included. This
        // mirrors exactly how _Version stamps the file.
        var numericVersion = releaseVersion.Split('-')[0];
        var expectedByAttribute = new Dictionary<string, string>
        {
        ...
        foreach (var pair in expectedByAttribute)
        {
            ...
            {
                errors.Add(
                    $"src/CommonAssemblyInfo.cs {attribute} is '{match.Groups[1].Value}' but for --version " +
                    $"'{releaseVersion}' it must be '{expected}' (AssemblyVersion/AssemblyFileVersion carry the " +
                    "numeric core only; AssemblyInformationalVersion carries the full string). Bump the version " +
                    "files in their own commit and release the merged version; the release input never overrides " +
                    "what is committed.");
            }
        }
    }
...
///////////////////////////////////////////////////////////////////////////////
// Assertion 1b: the Unity plugin assembly bundled into the .unitypackage was
// built at the version being released. _Package_Unity zips the committed
// unity/Assets/Ably tree as-is; it does not rebuild the plugin, so a stale
// unity/Assets/Ably/Plugins/Ably.PubSub.Device.dll (regenerated by the separate
// ./unity-plugins-updater.sh step) would otherwise ship inside the
// .unitypackage at the release label while every .nupkg is correct.
///////////////////////////////////////////////////////////////////////////////
...
public void ReleaseAssertUnityPluginVersion(string releaseVersion, List<string> errors)
{
    ...
    {
        errors.Add(
            $"{pluginDll.FullPath} is missing. The .unitypackage bundles this merged plugin assembly, and it " +
            "is produced by a separate step (./unity-plugins-updater.sh <version>). Regenerate and commit it " +
            "before releasing, or the Unity package ships without its core.");
        return;
    }

    var numericVersion = releaseVersion.Split('-')[0];

    // AssemblyVersion is the numeric identity ILRepack carries over from the
    // netstandard2.0 core via /attr; compare its MAJOR.MINOR.PATCH to the
    // numeric core of the release (prerelease-safe: identity is never labelled).
    var identity = System.Reflection.AssemblyName.GetAssemblyName(pluginDll.FullPath).Version;
    var identityCore = $"{identity.Major}.{identity.Minor}.{identity.Build}";
    if (identityCore != numericVersion)
    {
        errors.Add(
            $"unity/Assets/Ably/Plugins/Ably.PubSub.Device.dll has AssemblyVersion '{identity}' (core " +
            $"'{identityCore}') but --version is '{releaseVersion}'. The bundled Unity plugin is stale; " +
            $"rebuild it with ./unity-plugins-updater.sh {releaseVersion} and commit it.");
    }

    // The informational version carries the full SemVer2 string, prerelease label
    // included. Strip any +build metadata before comparing.
    var informational = System.Diagnostics.FileVersionInfo.GetVersionInfo(pluginDll.FullPath).ProductVersion;
    var informationalCore = (informational ?? string.Empty).Split('+')[0];
    if (informationalCore != releaseVersion)
    {
        errors.Add(
            $"unity/Assets/Ably/Plugins/Ably.PubSub.Device.dll AssemblyInformationalVersion is '{informational}' " +
            $"but --version is '{releaseVersion}'. Rebuild the Unity plugin with " +
            $"./unity-plugins-updater.sh {releaseVersion} and commit it.");
    }
}
    ...
    .Does(() =>
{
    ...
    Information($"  --version, src/CommonAssemblyInfo.cs and unity/Assets/Ably/version.txt agree on {releaseVersion} (assembly identity attributes carry its numeric core)");
